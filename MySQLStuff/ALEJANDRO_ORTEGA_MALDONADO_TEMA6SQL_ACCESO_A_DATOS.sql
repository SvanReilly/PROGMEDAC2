...
SELECT F.TITLE AS TITULO, COUNT(R.RENTAL_ID) AS VECES_ALQUILADA FROM FILM F
LEFT JOIN INVENTORY I ON F.FILM_ID = I.FILM_ID
RIGHT JOIN RENTAL R ON I.INVENTORY_ID= R.INVENTORY_ID
GROUP BY F.FILM_ID, TITULO ORDER BY VECES_ALQUILADA DESC;

#Ejercicio 29: Muestra el dinero recaudado por cada tienda. PAYMENT STAFF STORE
SELECT S.STORE_ID, SUM(P.AMOUNT) AS DINERO_GANADO_POR_TIENDA FROM STORE S
LEFT JOIN STAFF ST ON S.STORE_ID = ST.STORE_ID
RIGHT JOIN PAYMENT P ON ST.STAFF_ID = P.STAFF_ID
GROUP BY ST.STORE_ID
ORDER BY DINERO_GANADO_POR_TIENDA DESC;

#Ejercicio 30: Media de duración de películas con PG.
SELECT AVG(F.LENGTH) AS DURACION_MEDIA_PG FROM FILM F
WHERE RATING LIKE "PG";


#Ejercicio 31: Películas con mayor y con menor duración.
SELECT F.TITLE AS TITULO, F.LENGTH AS DURACION FROM FILM F
ORDER BY LENGTH ASC LIMIT 5; -- LAS 5 CON MENOR DURACION

SELECT F.TITLE AS TITULO, F.LENGTH AS DURACION FROM FILM F
ORDER BY LENGTH DESC LIMIT 5; -- LAS 5 CON MAYOR DURACION

#Ejercicio 32: Media de duración de las películas de cada categoría.
SELECT CA.NAME AS CATEGORIA, AVG(F.LENGTH) AS DURACION_MEDIA FROM FILM F
LEFT JOIN FILM_CATEGORY FC ON F.FILM_ID = FC.FILM_ID
